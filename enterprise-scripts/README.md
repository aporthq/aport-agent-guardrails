...
For IT review or direct upload into a device-management tool, download the bundled script and edit only the configuration block at the top:

```bash
curl -fsSL "https://api.aport.io/enterprise/scripts/deploy" -o aport-device-deploy.sh
```

The saved `aport-device-deploy.sh` is the single-file script to review, paste, or upload. It is generated by `scripts/bundle-enterprise-scripts.sh` and includes the shared Node.js core inline.
...
Native Windows PowerShell entrypoints are available from the same script service:

```powershell
Invoke-WebRequest -Uri "https://api.aport.io/enterprise/scripts/deploy.ps1" -OutFile "aport-device-deploy.ps1"
```

Unversioned URLs resolve to the latest GitHub release. For change-controlled rollouts, pin a reviewed release by adding `?version=<version>`.
