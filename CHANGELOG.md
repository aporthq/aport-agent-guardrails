...
- **Codex tool routing:** `webrun`, `local_shell`, `write_stdin`, `update_plan`, `request_user_input`, `computer_use`, the memory operators and other current Codex tools were denied with `oap.unknown_tool`. The adapter is now strict by default: unlisted tool names still deny with `oap.unknown_tool`, with no name-pattern inference. Operators who have reviewed their tool surface can set `APORT_CODEX_TOOL_FALLBACK=on` to route one clear payload effect (`url`, `path`/`file_path`, or `command`/`cmd`/`script`) to the matching policy; mixed effects deny with `oap.invalid_tool_arguments`.
