...
| **Codex CLI** | `codex` | [docs/frameworks/codex.md](docs/frameworks/codex.md) | Beta command hook: repo-local `.codex/hooks.json` for `PreToolUse`; Bash, `apply_patch`, MCP, and local function tools. State stays in `~/.aport/codex`. **Beta limits:** `apply_patch` calls touching more than one file and every `Glob`/`List`/`LS`/`LSP` call are denied by the hook, and warn mode does not lift those two denials. `webrun`, `browser` navigation, `image_gen.imagegen`, `local_shell`, `write_stdin`, `update_plan`, `request_user_input`, `request_user_input_async`, provider clock/context/wait/user-message/permission prompt tools, observed read-only Codex memory/goal/skill tools, collaboration tools, `computer_use`, MCP resource tools, and plugin-install requests are mapped. Persistent or unclassified memory writes such as `memories.add_ad_hoc_note` or `memory_write` fail closed until policy-backed. Local `computer_use` and interactive browser actions fail closed until hosted `web.browser.v1` can decide them, and `computer_use` must provide an explicit string action in every mode. Unlisted tool names deny as `oap.unknown_tool` by default. CI and pre-push compare APort's adapter with provider-derived tool names from `openai/codex`. Reviewed deployments can set `APORT_CODEX_TOOL_FALLBACK=on` to route one clear payload effect (`url`, path, or command); mixed effects deny. `write_stdin` is evaluated as complete shell input, while conflicting stdin aliases, partial chunks, shell continuations and control-only chunks fail closed. | `npx @aporthq/aport-agent-guardrails codex` |
