...
Codex web/search tools are authorized only when the payload carries a concrete URL or domain. Search-only payloads, such as `web.run` with only `search_query`, fail closed with `oap.missing_required_context` instead of forwarding raw queries or sending invalid `web.fetch.v1` context to the hosted verifier. In hosted mode, `web.fetch.v1` still requires a concrete URL because that is the policy schema the API validates.

`write_stdin` submits keystrokes into a session that `exec_command` or `unified_exec` opened. When that session is an interactive shell, the keystrokes are a new command, so the `chars` are evaluated against `system.command.execute` rather than waved through with the bookkeeping tools. A non-empty chunk must contain complete, non-whitespace terminal input. Conflicting stdin aliases deny with `oap.invalid_tool_arguments`; partial chunks, shell continuation syntax such as a trailing backslash or an open quote, and whitespace/control-only chunks deny with `oap.partial_stdin_unsupported`, because APort cannot authorize split shell input or prove that a newline will not execute already-buffered text. The cost is that keystrokes bound for a pager or a REPL are judged by the command policy too, which can deny input no shell would have run.
