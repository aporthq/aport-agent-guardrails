...
| `TodoRead`, `ToolSearch`, `list_available_plugins_to_install`, `update_plan`, `request_user_input`, `request_user_input_async`, `send_message_to_user_async`, `request_permissions`, `wait`, `wait_for_environment`, `get_context_remaining`, `new_context`, `clock.*`, goal tools, `memories.list`/`read`/`search`, `skills.*`, `memory_operators` and other `memory_*` tools | allowed without evaluator: session bookkeeping, plan updates, prompts to the user, host-mediated permission prompts, context/window controls, and Codex-owned memory/goal/skill metadata |
