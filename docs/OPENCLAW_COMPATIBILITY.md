...
| Return shape | `{ params?, block?, blockReason?, requireApproval? }`. `block: true` is terminal and skips lower-priority handlers. `block: false` is treated as no decision. | `{}` on allow. `{ block: true, blockReason: "<notice>" }` on deny, on unmapped tools, on integrity failure, and on evaluator error when `failClosed` is true. Warn/report-only mode downgrades only completed policy denials; hook schema drift, decision-integrity failures, and evaluator/runtime failures still block when `failClosed` is true. | OK |
