...
test:
	...
	@chmod +x bin/*.sh bin/lib/*.sh bin/frameworks/*.sh enterprise-scripts/*.sh enterprise-scripts/*.mjs scripts/*.sh tests/*.sh tests/unit/*.sh tests/frameworks/*/*.sh 2>/dev/null || true
	@APORT_SKIP_REMOTE_PASSPORT_TEST="$${APORT_SKIP_REMOTE_PASSPORT_TEST-1}" bash tests/run.sh
	@echo "✅ Tests complete"
