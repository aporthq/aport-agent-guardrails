...
- Codex tool routing (`bin/lib/command-hook-adapter.sh`): `webrun`, `local_shell`, `write_stdin`, `update_plan`, `request_user_input`, `computer_use`, `memory_*` and the collaboration tools are listed explicitly. Unlisted names are routed by whole name segments (`shell`/`exec` to command, `web`/`browser` to web, `patch`/`write` to file write, `read`/`grep` to file read, `agent`/`spawn` to session) before `oap.unknown_tool`; `APORT_CODEX_TOOL_FALLBACK=off` disables segment routing.
