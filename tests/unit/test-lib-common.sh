...
out=$(log_warn "w" 2>&1)
[[ "$out" == *"WARN"* ]] && [[ "$out" == *"w"* ]] || { echo "FAIL: log_warn" >&2; exit 1; }
echo "  ✅ log_warn"

# log_error contains ERROR
out=$(log_error "e" 2>&1)
[[ "$out" == *"ERROR"* ]] && [[ "$out" == *"e"* ]] || { echo "FAIL: log_error" >&2; exit 1; }
echo "  ✅ log_error"
...
[[ "$exitcode" -ne 0 ]] || { echo "FAIL: require_cmd should exit non-zero" >&2; exit 1; }
[[ "$out" == *"ERROR"* ]] && [[ "$out" == *"not found"* ]] || { echo "FAIL: require_cmd error message" >&2; exit 1; }
