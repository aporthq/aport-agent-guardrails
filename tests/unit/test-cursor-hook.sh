...
EXIT1=$?
[[ "$EXIT1" -eq 0 ]] || { echo "FAIL: expected exit 0 for allow, got $EXIT1" >&2; exit 1; }
grep -q '"allowed":true' "$OUT1" || { echo "FAIL: output should contain allowed:true" >&2; cat "$OUT1" >&2; exit 1; }
grep -q '"permission":"allow"' "$OUT1" || { echo "FAIL: output should contain permission:allow" >&2; exit 1; }
...
EXIT2=$?
set -e
[[ "$EXIT2" -eq 2 ]] || { echo "FAIL: expected exit 2 for deny, got $EXIT2 (output: $(cat "$OUT2"))" >&2; exit 1; }
grep -q '"allowed":false' "$OUT2" || { echo "FAIL: output should contain allowed:false" >&2; cat "$OUT2" >&2; exit 1; }
grep -q '"permission":"deny"' "$OUT2" || { echo "FAIL: output should contain permission:deny" >&2; exit 1; }
...
EXIT3=$?
[[ "$EXIT3" -eq 0 ]] || { echo "FAIL: expected exit 0 for npm install, got $EXIT3" >&2; exit 1; }
grep -q '"allowed":true' "$OUT3" || { echo "FAIL: Copilot-style allow" >&2; exit 1; }
...
EXIT4=$?
[[ "$EXIT4" -eq 0 ]] || { echo "FAIL: empty stdin should exit 0 (fail-open)" >&2; exit 1; }
grep -q '"allowed":true' "$OUT4" || { echo "FAIL: empty stdin allow" >&2; exit 1; }
