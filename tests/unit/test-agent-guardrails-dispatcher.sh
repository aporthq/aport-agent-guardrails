...
[[ "$DISPATCHER_EXIT" -ne 0 ]] || { echo "FAIL: expected non-zero" >&2; exit 1; }
grep -q "Invalid agent_id format" "$out1" || { echo "FAIL: openclaw should receive invalid_agent_id" >&2; cat "$out1" >&2; exit 1; }
...
[[ "$DISPATCHER_EXIT" -ne 0 ]] || { echo "FAIL: expected non-zero" >&2; exit 1; }
grep -q "Invalid agent_id format" "$out2" || { echo "FAIL: -f openclaw pass-through" >&2; exit 1; }
...
set +e
printf 'openclaw\n' | (unset CI APORT_NONINTERACTIVE; cd "$EMPTY_DIR" && "$DISPATCHER" openclaw invalid_agent_id >"$out3" 2>&1)
DISPATCHER_EXIT=$?
set -e
[[ "$DISPATCHER_EXIT" -ne 0 ]] || { echo "FAIL: expected non-zero" >&2; exit 1; }
grep -q "Invalid agent_id format" "$out3" || { echo "FAIL: openclaw should get args from empty cwd" >&2; cat "$out3" >&2; exit 1; }
...
[[ "$DISPATCHER_EXIT" -ne 0 ]] || { echo "FAIL: unknown framework should exit 1" >&2; exit 1; }
grep -qi "unknown\|unsupported\|nosuchframework" "$out4" || { echo "FAIL: error message" >&2; exit 1; }
echo "  ✅ unknown framework -> exit 1"

# 5. -f without value -> exit 1
out5="$TEST_DIR/dispatcher-5.txt"
run_dispatcher "$out5" "" -f
[[ "$DISPATCHER_EXIT" -ne 0 ]] || { echo "FAIL: -f without value" >&2; exit 1; }
grep -q "requires a value\|ERROR" "$out5" || { echo "FAIL: -f error message" >&2; exit 1; }
...
e6=$?
set -e
[[ "$e6" -ne 0 ]] || { echo "FAIL: non-interactive no detection should exit 1" >&2; exit 1; }
grep -q "No framework detected\|APORT_FRAMEWORK\|--framework" "$out6" || { echo "FAIL: non-interactive message" >&2; cat "$out6" >&2; exit 1; }
...
e7=$?
set -e
[[ "$e7" -ne 0 ]] || { echo "FAIL: openclaw with invalid_agent_id should exit 1" >&2; exit 1; }
grep -q "Invalid agent_id format" "$out7" || { echo "FAIL: APORT_FRAMEWORK=openclaw should run openclaw" >&2; cat "$out7" >&2; exit 1; }
...
e8=$?
set -e
[[ "$e8" -ne 0 ]] || { echo "FAIL: non-interactive conflict should exit 1" >&2; exit 1; }
grep -q "Multiple frameworks detected" "$out8" || { echo "FAIL: conflict message" >&2; cat "$out8" >&2; exit 1; }
grep -q "langchain" "$out8" && grep -q "openclaw" "$out8" || { echo "FAIL: conflict should list both" >&2; exit 1; }
