...
[[ "$got" == *".aport"* ]] && [[ "$got" == *"langchain"* ]] || {
    echo "FAIL: get_config_dir langchain" >&2
    exit 1
}
...
[[ "$got" == *"crewai"* ]] || {
    echo "FAIL: get_config_dir crewai" >&2
    exit 1
}
...
[[ "$got" == *"cursor"* ]] || {
    echo "FAIL: get_config_dir cursor" >&2
    exit 1
}
...
[[ "$got" == *"/aport/passport.json"* ]] && [[ "$got" == *"cursor"* ]] || {
    echo "FAIL: get_default_passport_path cursor" >&2
    exit 1
}
got=$(get_default_passport_path openclaw)
[[ "$got" == *"/aport/passport.json"* ]] && [[ "$got" == *"openclaw"* ]] || {
    echo "FAIL: get_default_passport_path openclaw" >&2
    exit 1
}
...
[[ -n "$written" ]] || {
    echo "FAIL: write_config_template should echo path" >&2
    exit 1
}
[[ -d "$written" ]] || {
    echo "FAIL: write_config_template should create dir: $written" >&2
    exit 1
}
[[ "$written" == *"langchain"* ]] || {
    echo "FAIL: write_config_template path" >&2
    exit 1
}
...
[[ -d "$written2" ]] || {
    echo "FAIL: write_config_template n8n: $written2" >&2
    exit 1
}
